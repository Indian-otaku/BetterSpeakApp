...
class BetterSpeakApp(QMainWindow):
    ...
    def __init__(self):
        ...
        logo_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "betterspeaklogo.ico")
    ...
    def graph_buttons(self):
        ...
        self.page1_row4_graph_button_part = QPushButton("")
    ...
    def pss_calculation(self):  
        ...
        if getattr(self, 'modelThreads', None):
            for thread in self.modelThreads:
                if thread.isRunning():
                    return
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
    ...
    def create_lpage(self):
        
        # Design part 

        self.lpage = QWidget()
        self.lpage.setObjectName("lpage")
        self.lpage_layout = QVBoxLayout(self.lpage)

        self.lpage_layout.addWidget(QLabel("'Damn this is great application' - the creator himself"))


        self.stacked_widgets.addWidget(self.lpage)

    def show_lpage(self):
        self.create_lpage()
        self.stacked_widgets.setCurrentWidget(self.lpage)
